    ...
    const addCollection = function () {
        ...
        const callback = function (response) {
            ...
            if (response.status === 201) {
                ...
                response.json().then(function (data) {
                    domModule.html('#message', '<div class="alert alert-success">Collection created ( <a href="' + configModule.getApi() + '/dashboard/objects/?pid=' + DOMPurify.sanitize(data.uuid) + '">' + DOMPurify.sanitize(data.uuid) + '</a> )');
                    domModule.hide('#collection-form');
                });
