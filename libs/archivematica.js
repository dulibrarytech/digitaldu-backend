...
const Archivematica = class {
    ...
    async start_transfer(collection_uuid, archival_package) {
        ...
        try {
            ...
            let location = transfer_source + ':' + sftp_path + '/' + collection_uuid + '/' + archival_package;
            ...
            const name = collection_uuid + '_' + archival_package + '_transfer';
            const type = 'standard';
            const data_qs = `name=${name}&type=${type}&accession=&paths%5B%5D=${encoded_location}&rows_ids%5B%5D=%5B%22%22%5D`;
            const response = await HTTP.post(endpoint, data_qs, {
                headers: {
    ...
    /**
     * Gets unapproved transfer list
     * @return unapproved_transfer_list
     */
    async get_unapproved_transfer_list() {
        ...
        try {
            ...
            let endpoint = this.ARCHIVEMATICA.archivematica_api;
            endpoint += 'transfer/unapproved/?username=';
            endpoint += this.ARCHIVEMATICA.archivematica_username;
            endpoint += '&api_key=';
            endpoint += this.ARCHIVEMATICA.archivematica_api_key;
            ...
            LOGGER.module().error('ERROR: [/libs/archivematica lib (approve_transfer)] unable to get unapproved transfer list. Request failed: ' + error.error);
        }
    };
    ...
    async approve_transfer(transfer_folder) {
        ...
        try {
            ...
            const data_qs = `type=standard&directory=${transfer_folder}`;
            const response = await HTTP.post(endpoint, data_qs, {
                headers: {
                    'Content-Type': 'application/x-www-form-urlencoded'
                }
