...
const TRANSCRIPTS = require('../libs/transcripts');
...
exports.restore_compound_parts = function (uuid, callback) {
    ...
    try {
        ...
        (async () => {
            ...
            .select('display_record', 'compound_parts')
            .where({
            ...
            let timer = setInterval(async () => {
                ...
                let metadata = JSON.parse(record.display_record);
                let compound_parts = JSON.parse(record.compound_parts);

                if (metadata.parts !== undefined) {

                    let updated_metadata = metadata;
                    delete updated_metadata.parts;
                    updated_metadata.display_record.parts = compound_parts;

                    let is_updated = await DB(DB_TABLES.repo.repo_records)
                    .where({
                        pid: metadata.pid
                    })
                    .update({
                        display_record: JSON.stringify(updated_metadata),
                        is_complete: 3
                    });
                ...
                console.log('Looking for broken parts array...');
    ...
    callback({
        status: 200,
        message: 'Restoring compound parts'
    });
};
